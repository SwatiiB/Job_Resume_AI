...
const RecruiterHeader = ({ 
  ...
  const recentApplications = myJobs.reduce((sum, job) => sum + (job.applicationCount || 0), 0);
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="hidden md:block relative">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 w-80 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-colors"
          />
        </div>
        ...
        <div className="hidden lg:flex items-center space-x-6 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-gray-600">{myJobs.length} jobs</span>
          </div>
          <div className="flex items-center space-x-2">
            <Users className="w-4 h-4 text-secondary-500" />
            <span className="text-gray-600">{recentApplications} applications</span>
          </div>
        </div>
      </div>
                  ...
                  notifications.slice(0, 5).map((notification) => (
                    <div
                      ...
                      className={cn(
                        ...
                        notification.readAt ? "border-transparent" : "border-accent-500 bg-accent-50/30"
                      )}
                    >
                      <div className="flex items-start space-x-3">
                        <div className={cn(
                          "w-2 h-2 rounded-full mt-2",
                          notification.readAt ? "bg-gray-300" : "bg-accent-500"
                        )} />
            ...
            >
              <div className="px-4 py-2 border-b border-gray-100">
                ...
                {profile?.company?.name && (
                  <p className="text-xs text-accent-600 font-medium">{profile.company.name}</p>
                )}
              </div>
